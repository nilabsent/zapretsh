...
- `TCP_PORTS` - порты TCP, для фильтрации исходящего трафика посредством стратегий. Диапазоны пишутся через `:`, порты разделяются запятой. Значение по умолчанию `80,443` (http и https)
- `UDP_PORTS` - порты UDP, для фильтрации исходящего трафика посредством стратегий. Диапазоны пишутся через `:`, порты разделяются запятой. Значение по умолчанию `443,50000:50099` (quic и голосовой трафик Discord)
